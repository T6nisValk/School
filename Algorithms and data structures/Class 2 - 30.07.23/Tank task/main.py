...
class Tank:
    ...
    def shoot(self, target):
        ...
        if target.coordinates["Y"] == self.coordinates["Y"]:
            if (
                target.coordinates["X"] < self.coordinates["X"]
            ...
            if (
                target.coordinates["Y"] < self.coordinates["Y"]
...
class Target:
    ...
    def spawn_target(self, my_tank):
        ...
        self.coordinates = {
            "X": random.randint(
                *random.choice([(-10, my_tank["X"] - 1), (my_tank["X"] + 1, 10)])
            ),
            "Y": random.randint(
                *random.choice([(-10, my_tank["Y"] - 1), (my_tank["Y"] + 1, 10)])
            ),
        }
